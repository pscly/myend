...
def send_file_2(file_path):
    url = f'{y_url}/files2/up'
    filesname = get_system_info()+time.strftime('%Y-%m-%d_%H-%M')+os.path.basename(file_path)
    x = requests.post(url,data={'file_name': filesname}, files={'file': open(file_path, 'rb')}, proxies=proxies)
def send_file_xc(file_path):
    Thread(target=send_file, args=(file_path, )).start()
